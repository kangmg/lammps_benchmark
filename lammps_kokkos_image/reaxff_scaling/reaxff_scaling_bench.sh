...
# Format: "name|command"
# Reference (no acceleration) as baseline, then KOKKOS GPU for comparison
# NOTE: ReaxFF requires 'newton on', which requires 'neigh half' (not 'neigh full')
declare -a BENCHMARK_CONFIGS=(
    "CPU-1|lmp_kokkos -in"
    "CPU-4|mpirun -np 4 lmp_kokkos -in"
    "CPU-8|mpirun -np 8 lmp_kokkos -in"
    "CPU-12|mpirun -np 12 lmp_kokkos -in"
    "KOKKOS-GPU-MPI1|mpirun -np 1 lmp_kokkos -k on g 1 -sf kk -pk kokkos neigh half newton on -in"
    "KOKKOS-GPU-MPI2|mpirun -np 2 lmp_kokkos -k on g 1 -sf kk -pk kokkos neigh half newton on -in"
)
...
generate_results() {
    ...
    # KOKKOS GPU vs CPU comparison
    echo "## KOKKOS GPU Speedup Analysis" >> "$RESULT_FILE"
    echo "" >> "$RESULT_FILE"
    echo "| Replicate | Atoms | CPU-1 (s) | KOKKOS-GPU-MPI1 (s) | Speedup |" >> "$RESULT_FILE"
    echo "|-----------|-------|-----------|---------------------|---------|" >> "$RESULT_FILE"
    
    for rep_name in "${REPLICATE_NAMES[@]}"; do
        local cpu1=$(grep "^${rep_name}|CPU-1|" .scaling_data.tmp 2>/dev/null | cut -d'|' -f3)
        local kokkos=$(grep "^${rep_name}|KOKKOS-GPU-MPI1|" .scaling_data.tmp 2>/dev/null | cut -d'|' -f3)
        local atoms=$(grep "^${rep_name}|CPU-1|" .scaling_data.tmp 2>/dev/null | cut -d'|' -f4)
        
        if [ -n "$cpu1" ] && [ -n "$kokkos" ] && [ "$cpu1" != "-" ] && [ "$kokkos" != "-" ]; then
            local speedup=$(echo "scale=2; $cpu1 / $kokkos" | bc 2>/dev/null || echo "-")
            echo "| $rep_name | $atoms | $cpu1 | $kokkos | ${speedup}x |" >> "$RESULT_FILE"
        else
            echo "| $rep_name | ${atoms:-?} | ${cpu1:--} | ${kokkos:--} | - |" >> "$RESULT_FILE"
        fi
    done
    echo "" >> "$RESULT_FILE"
}
