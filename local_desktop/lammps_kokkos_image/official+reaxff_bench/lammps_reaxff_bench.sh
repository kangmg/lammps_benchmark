...
show_usage() {
    cat << 'EOF'
Usage: ./run_lammps_benchmark.sh [options]
...
Examples:
  # Reference baseline (no acceleration) - for comparison
  ./lammps_reaxff_bench.sh -c "Reference|lmp_kokkos -in"
  
  # MPI only (CPU parallel, no GPU)
  ./lammps_reaxff_bench.sh -c "MPI-8|mpirun -np 8 lmp_kokkos -in"
  
  # KOKKOS GPU acceleration (recommended for ReaxFF - includes QEq GPU acceleration)
  # NOTE: ReaxFF requires newton on, which requires neigh half (not neigh full)
  ./lammps_reaxff_bench.sh \
    -c "KOKKOS-GPU|mpirun -np 1 lmp_kokkos -k on g 1 -sf kk -pk kokkos neigh half newton on -in"
  
  # Compare Reference vs KOKKOS (ReaxFF performance comparison)
  ./lammps_reaxff_bench.sh \
    -c "Reference|lmp_kokkos -in" \
    -c "MPI-8|mpirun -np 8 lmp_kokkos -in" \
    -c "KOKKOS-GPU|mpirun -np 1 lmp_kokkos -k on g 1 -sf kk -pk kokkos neigh half newton on -in"

  # Full ReaxFF benchmark suite
  ./lammps_reaxff_bench.sh \
    -c "CPU-1|lmp_kokkos -in" \
    -c "CPU-4|mpirun -np 4 lmp_kokkos -in" \
    -c "CPU-8|mpirun -np 8 lmp_kokkos -in" \
    -c "CPU-12|mpirun -np 12 lmp_kokkos -in" \
    -c "KOKKOS-GPU-MPI1|mpirun -np 1 lmp_kokkos -k on g 1 -sf kk -pk kokkos neigh half newton on -in" \
    -c "KOKKOS-GPU-MPI2|mpirun -np 2 lmp_kokkos -k on g 1 -sf kk -pk kokkos neigh half newton on -in"

Note: 
  - The command should end with '-in' as the input file will be appended automatically.
  - ReaxFF requires 'newton on', which requires 'neigh half' (not 'neigh full')
  - KOKKOS GPU options: -k on g 1 (1 GPU), -sf kk (KOKKOS suffix), -pk kokkos neigh half newton on
EOF
}
...
# 2x2x2 replicate (~6720 atoms), 100 timesteps
...
run_benchmark() {
    ...
    local command=$3         # e.g., "mpirun -np 20 lmp_serial -in"
    local input_file=$4      # e.g., "in.lj"
    ...
    echo "Command: $command $input_file"
    echo ""
    ...
    # The command is expected to end with -in, we append input file and log options
...
EOF
    ...
    for bench_type in lj eam chain rhodo reaxff; do
        if grep -q "^${bench_type}|" .benchmark_data.tmp 2>/dev/null; then
            # Get baseline (first entry for this benchmark)
            local baseline=$(grep "^${bench_type}|" .benchmark_data.tmp | head -1)
            IFS='|' read -r bt desc baseline_time rest <<< "$baseline"
            
            if [ -z "$baseline_time" ] || [ "$baseline_time" = "-" ]; then
                continue
            fi
            
            echo "### ${bench_type^^} Speedup" >> "$RESULT_FILE"
            echo "" >> "$RESULT_FILE"
            echo "Baseline: $(echo "$desc" | sed 's/^.*- //') ($baseline_time seconds)" >> "$RESULT_FILE"
            echo "" >> "$RESULT_FILE"
            echo "| Configuration | Time (s) | Speedup |" >> "$RESULT_FILE"
            echo "|---------------|----------|---------|" >> "$RESULT_FILE"
            
            grep "^${bench_type}|" .benchmark_data.tmp | while IFS='|' read -r bt desc time rest; do
                config=$(echo "$desc" | sed 's/^.*- //')
                
                if [ -z "$time" ] || [ "$time" = "-" ]; then
                    speedup="-"
                else
                    speedup=$(echo "scale=2; $baseline_time / $time" | bc 2>/dev/null || echo "-")
                fi
                
                echo "| $config | $time | ${speedup}x |" >> "$RESULT_FILE"
            done
...
main() {
    ...
    for i in "${!bench_types[@]}"; do
        ...
        for config in "${BENCHMARK_CONFIGS[@]}"; do
            IFS='|' read -r name command <<< "$config"
            run_benchmark "$bench_type" "$name" "$command" "$input_file"
        done
