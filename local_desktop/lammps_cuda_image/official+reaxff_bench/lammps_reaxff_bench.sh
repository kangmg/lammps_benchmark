...
Examples:
  # Test single binary in serial mode
  ./run_lammps_benchmark.sh -c "Serial|lmp -in"
  
  # Test with MPI
  ./run_lammps_benchmark.sh -c "MPI-20|mpirun -np 20 lmp_serial -in"
  
  # Compare multiple configurations
  ./run_lammps_benchmark.sh \
    -c "Serial|lmp_serial -in" \
    -c "MPI-10|mpirun -np 10 lmp_serial -in" \
    -c "MPI-20|mpirun -np 20 lmp_serial -in" \
    -c "GPU|lmp_gpu -sf gpu -pk gpu 1 -in"

  # Complex example with different binaries
  ./run_lammps_benchmark.sh \
    -c "CPU-Serial|/path/to/lmp_cpu -in" \
    -c "CPU-MPI-40|mpirun -np 40 /path/to/lmp_cpu -in" \
    -c "GPU-Single|/path/to/lmp_gpu -sf gpu -pk gpu 1 -in" \
    -c "KOKKOS-OpenMP|/path/to/lmp_kokkos -sf kk -k on t 8 -in"
